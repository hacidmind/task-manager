...
export async function getUser(req) {
  ...
  const user = await db.collection("users").findOne({ _id: session.userId }, { projection: { name: 1, email: 1, passwordSetupAt: 1, workspaceName: 1, avatar: 1, theme: 1, statuses: 1 } });
  ...
  return user ? {
    ...
    workspaceName: user.workspaceName || "Virtual Card",
    avatar: user.avatar || "",
